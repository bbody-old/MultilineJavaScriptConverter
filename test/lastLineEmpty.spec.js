  ...
  describe('ECMA5 Single Quote', () => {
    it('should trim out the last line', () => {
      const multiLineContentExpected =
        '\'Something\' +\n\t\'multi\' +\n\t\'line\'';
      let results = stringConverter.convertText(
        'someVariable', sampleLine, ECMA5_SINGLE, false, true, true, 'tabs'
      );
      assert.equal(
  ...
  describe('ECMA5 Double Quote', () => {
    it('should trim out the last line', () => {
      const multiLineContentExpected =
        '"Something" +\n\t"multi" +\n\t"line"';
      let results = stringConverter.convertText(
        'someVariable', sampleLine, ECMA5_DOUBLE, false, true, true, 'tabs'
      );
      assert.equal(
        results, `var someVariable = ${multiLineContentExpected};`
      );
